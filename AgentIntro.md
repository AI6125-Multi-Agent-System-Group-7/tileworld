...
## Agent 2 - AgentHanny
AgentHanny 是在 Group7AgentBase 上实现的策略代理，当前正在测试我组会上提出的生存模型，Agent主打一个“计划经济”。\
...
- 对候选plan按存活概率与距离排序；若 `p_alive < p_theta` 或超期则懒删除记忆并跳过。
...
7. 多少个‘蒸蚌！’历史统计：
- 实验设置：环境里创建6个同样的‘影分身の術’然后运行5000回合数次，取大致均值（现在基类Agent以及不可能找不到猫粮了）
- v1 - 活着优先 （在Group7BaseAgent旧版本上的最小实现）
  - Final Reward: ~465.
- v2 - 蝗虫过境（指数衰减建模+hazard-aware的贪婪式计划）
  - Final Reward: Para1 ~1200; Para2 ~1400.
- v3 - 计划经济（指数衰减建模+hazard-aware的计划式行动）
  - Final Reward: Para1 ~1300; Para2 ~1500.
